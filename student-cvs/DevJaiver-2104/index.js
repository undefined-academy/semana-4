...
function verificaIncluidos(entry){
    return _.includes(entry.tags, "Desarrollo")
};

function mapearDatos (entry){
    return _.map(entry, "titulo");
};

const filtros = _.filter(entries, verificaIncluidos);

const agrupacionPorAutor = _.groupBy(filtros, "autor");



const MapPorTitulo = _.mapValues(agrupacionPorAutor, mapearDatos );



console.log(MapPorTitulo);