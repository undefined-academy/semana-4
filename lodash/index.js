...
const filteredByTag = _.filter(entries, function(entry) {
  return _.includes(entry.tags, "Desarrollo")
});
const groupedByAuthor = _.groupBy(filteredByTag, "autor");

const mappedByTitle = _.mapValues(groupedByAuthor, function(entry) {
  return _.map(entry, "titulo");
});

console.log(mappedByTitle);